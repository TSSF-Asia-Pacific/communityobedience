...
ZH_MTIME=$(grep "Exported at:" community-obedience-zh.xlf | cut -d ':' -f 2-) # need to rename -zh-cn
...
curl -o 'community-obedience-zh.xlf' -z "$ZH_MTIME" "https://localise.biz/api/export/locale/zh.xlf?index=text&key=$LOCALISE_KEY&format=xlf2&fallback=en-AU" # need to rename zh-cn
